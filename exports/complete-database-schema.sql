...
-- User department access (for cross-department access)
...
CREATE INDEX idx_maintenance_records_next_service_due ON public.maintenance_records(next_service_due);
...
-- Function to check if user is in a department
...
CREATE POLICY "Users can view reports in their department or created by them"
  ...
  USING (
    (created_by = auth.uid())
    OR user_in_department(auth.uid(), department_id)
    OR has_role(auth.uid(), 'admin')
    OR has_role(auth.uid(), 'director')
  );
  ...
  USING (
    EXISTS (
      ...
      AND (
        ...
        OR user_in_department(auth.uid(), r.department_id)
        OR has_role(auth.uid(), 'admin')
        OR has_role(auth.uid(), 'director')
      )
    )
...
CREATE POLICY "Users can view attachments for reports they can see"
